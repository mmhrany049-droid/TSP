/**
 * قالب‌بندی اعداد، درصد، مدت‌زمان و متن‌های کوتاه برای نمایش فارسی.
 *
 * همهٔ توابع این ماژول خالص‌اند (بدون وابستگی به مرورگر یا پایگاه داده)، پس در سرور
 * و مرورگر نتیجهٔ یکسان می‌دهند و به‌سادگی آزمون می‌شوند. رقم‌های فارسی از `lib/date`
 * می‌آید تا تبدیل اعداد فقط یک‌جا پیاده شده باشد.
 */
...
import { toPersianDigits } from "./date.ts";
